...
export default function PaymentModal({ customer, emis, breakdown, onClose, onSubmitted, isAdmin }: PaymentModalProps) {
  ...
  const collectableEmis = emis.filter(e => e.status === 'UNPAID' || (e.fine_amount || 0) > (e.fine_paid_amount || 0));

  // Safely get next emi no — fallback to first unpaid
  const defaultEmiNo = breakdown?.next_emi_no ?? unpaidEmis[0]?.emi_no ?? collectableEmis[0]?.emi_no ?? 0;
  ...
  // Find selected EMI from all sources
  const selectedEmi = unpaidEmis.find(e => e.emi_no === selectedEmiNo)
    || collectableEmis.find(e => e.emi_no === selectedEmiNo)
    || emis.find(e => e.emi_no === selectedEmiNo);

  const scheduledEmiAmount = selectedEmi?.amount ?? 0;
  const scheduledFine = breakdown?.fine_due ?? 0;
  const scheduledFirstEmiCharge = breakdown?.first_emi_charge_due ?? 0;
  ...
  // If no EMI is selected yet but unpaid EMIs exist, auto-select the first one
  useEffect(() => {
  ...
  async function handleSubmit() {
    console.log('handleSubmit called', { collectType, selectedEmi, isAdmin, mode, totalPayable, retailerPin: retailerPin ? '***' : 'EMPTY' });

    // Validation with clear messages
    if (collectType !== 'fine_only') {
      if (!selectedEmi) {
        toast.error('Please select an EMI to pay');
        return;
      }
      if (unpaidEmis.length === 0) {
        toast.error('No unpaid EMIs available');
        return;
      }
    }

    if (!isAdmin && !retailerPin.trim()) {
      toast.error('Enter your Retail PIN to submit payment');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || `Payment failed (status ${res.status})`);
      } else {
        ...
        <div className="p-5 space-y-5">
          ...
          {!isAdmin && (
            <div>
              ...
              <p className="text-xs text-ink-muted mt-1">Required to submit payment</p>
            </div>
          )}
          ...
          <div className="card bg-surface-2 p-4 space-y-3">
            ...
            {collectType !== 'fine_only' && scheduledEmiAmount > 0 && (
              <div>
            ...
            {scheduledFine > 0 && collectType !== 'emi_only' && (
              <div>
          ...
          {/* Actions — BUTTON IS NEVER DISABLED — validation happens on click */}

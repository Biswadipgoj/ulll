...
export default function RetailerDashboard() {
  ...
  // Broadcast messages
  const [broadcastPopups, setBroadcastPopups] = useState<{ id: string; message: string; image_url?: string | null; expires_at: string }[]>([]);
  ...
  async function selectCustomer(customer: Customer) {
    ...
    if (bdErr) {
      ...
      const fallback: DueBreakdown = {
        ...
        fine_due: 0,
        first_emi_charge_due: customer.first_emi_charge_paid_at ? 0 : (customer.first_emi_charge_amount || 0),
        total_payable: (nextUnpaid?.amount ?? 0) + (customer.first_emi_charge_paid_at ? 0 : (customer.first_emi_charge_amount || 0)),
        popup_first_emi_charge: !customer.first_emi_charge_paid_at && (customer.first_emi_charge_amount || 0) > 0,
        popup_fine_due: false,

-- ============================================================
-- MIGRATION 009: Final fixes for all sections
-- Run in Supabase SQL Editor (idempotent)
-- ============================================================

-- ── 1. Fine settings: add weekly_fine_increment ─────────────
...
-- ── 5. Updated fine engine using fine_settings.weekly_fine_increment ──
...
BEGIN
  ...
  -- Recalculate fines for this customer
...
-- ── 7. Updated EMI schedule generator with emi_start_date ───
...
-- ── 8. RLS for broadcast_messages — allow retailers to insert ──
...
-- ── 9. Run fine calculation ─────────────────────────────────
SELECT * FROM calculate_and_apply_fines();

-- ── 10. Cron job for daily fines ────────────────────────────
DO $$
BEGIN
...
DO $$ BEGIN
  ...
  RAISE NOTICE 'Added: weekly_fine_increment, UTR indexes, RLS updates, fine engine fixes.';
END $$;
